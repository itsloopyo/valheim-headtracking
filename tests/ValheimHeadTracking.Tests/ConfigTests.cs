...
namespace ValheimHeadTracking.Tests
{
    /// <summary>
    ...
    public class ConfigTests
    {
        ...
        {
            byte[] rendered = ValheimConfig.Table().RenderFresh(new RenderHeader(ValheimConfig.DisplayName));
        ...
        [Fact]
        public void DefaultsKeepTheShippedBehaviourButTheVerticalLimits()
        {
            ValheimConfig config = Defaults();
            ...
            // Both limits are global and follow Defaults.ini, whose built-in values are the
            // schema's 0.2. Earlier builds shipped 0.6 and 0.4; a player's .cfg carries its values.
        ...
        public void FirstLaunchCreatesTheCommittedBytes()
        {
            ...
            {
                ConfigLoadResult<ValheimConfig> loaded = Owner(dir.Path).Load();
        ...
        public void DefaultRowsFollowDefaultsIni()
        {
            using (var dir = new TempDir())
            {
                ...
                File.WriteAllText(defaults, text.Replace("UdpPort=4242", "UdpPort=4343")
                    .Replace("PositionLimitY=0.2", "PositionLimitY=0.6")
                    .Replace("WorldSpaceYaw=true", "WorldSpaceYaw=false"));

                ConfigLoadResult<ValheimConfig> next = Owner(dir.Path).Load();

                Assert.Equal(ConfigLoadStatus.Canonical, next.Status);
                Assert.Equal(4343, next.Config.UdpPort);
                Assert.Equal(0.6f, next.Config.Position.LimitY);
        ...
        public void TheYawToggleSavesOnlyItsRowAndItComesBack()
        {
            using (var dir = new TempDir())
            {
                ...
                ConfigOwner<ValheimConfig> owner = Owner(dir.Path);
                owner.Load();
        ...
        public void TheFileHasNoRowTheModDoesNotUse()
        {
            ...
            foreach (string gone in new[]
                     {
                         "Sensitivity", "Invert", "Reticle", "Crosshair", "AimDecoupling", "Recenter", "PositionToggleKey",
                         "PositionLimitX", "PositionLimitZ", "TrueFreeLook", "Collision", "Light", "TrackerPivot",
                         "PositionAllowed", "DataFreshness",
                     })
            {
        ...
        private static ValheimConfig Defaults()
        {
        ...
        private static ConfigOwner<ValheimConfig> Owner(string dir)
        {
        ...
        private sealed class TempDir : IDisposable
        {
            public TempDir()
            {
                Path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "vh-config-" + Guid.NewGuid().ToString("N"));
